...
def main():
    ...
    for agent in orchestrator._agents.values():
        print(
            "    - {:<30s}  type={:<15s}".format(agent.agent_id, agent.agent_type.value)
        )
    ...
    print(
        "  Avg quality (accepted)   E[p | accepted]    = {:.4f}".format(
            avg_quality_accepted
        )
    )
    print("  Spread                                      = {:+.4f}".format(spread_val))
    print(
        "  Uncertain fraction       (band=0.15)        = {:.4f}".format(uncertain_frac)
    )
    print(
        "  Conditional loss (init)                     = {:+.4f}".format(cond_loss_init)
    )
    print(
        "  Conditional loss (c.p.)                     = {:+.4f}".format(cond_loss_cp)
    )
    print()
    print(
        "  Total welfare (accepted)                    = {:.2f}".format(
            welfare["total_welfare"]
        )
    )
    print(
        "  Total social surplus                        = {:.2f}".format(
            welfare["total_social_surplus"]
        )
    )
    print(
        "  Avg initiator payoff                        = {:.4f}".format(
            welfare["avg_initiator_payoff"]
        )
    )
    print(
        "  Avg counterparty payoff                     = {:.4f}".format(
            welfare["avg_counterparty_payoff"]
        )
    )
    print()
    print("  CV(p)                                       = {:.4f}".format(cv["cv_p"]))
    print(
        "  CV(payoff_init)                             = {:.4f}".format(
            cv["cv_payoff_initiator"]
        )
    )
    print(
        "  CV(payoff_cp)                               = {:.4f}".format(
            cv["cv_payoff_counterparty"]
        )
    )
    ...
    print(
        "    High-quality acceptance rate  = {:.2%}  (n={})".format(
            participation["high_quality_acceptance"],
            participation["high_quality_count"],
        )
    )
    print(
        "    Low-quality acceptance rate   = {:.2%}  (n={})".format(
    ...
    for atype in ["Diligent", "Spam Bot", "Human Pretender", "Collusive Voter"]:
        ...
        print(
            "  Challenge passes      : {}".format(challenge_pass_by_type.get(atype, 0))
        )
        print(
            "  Challenge failures    : {}".format(challenge_fail_by_type.get(atype, 0))
        )
        ch_total = challenge_pass_by_type.get(atype, 0) + challenge_fail_by_type.get(
            atype, 0
        )
        if ch_total > 0:
    ...
    for agent_id in sorted(orchestrator._agents.keys()):
        ...
        print(
            "  {:<30s}  {:<20s}  {:10.4f}  {:8.1f}  {:9d}  {:7d}".format(
    ...
    if collusion_report is not None:
        print(
            "  Ecosystem collusion risk : {:.4f}".format(
                collusion_report.ecosystem_collusion_risk
            )
        )
        print(
            "  Flagged pairs            : {}".format(collusion_report.n_flagged_pairs)
        )
        print(
            "  Flagged groups           : {}".format(collusion_report.n_flagged_groups)
        )
        print()
        ...
        if (
            hasattr(collusion_report, "flagged_pairs")
            and collusion_report.flagged_pairs
        ):
            print()
            print("  Flagged collusive pairs:")
            for pair_report in collusion_report.flagged_pairs:
                a = getattr(pair_report, "agent_a", None) or getattr(
                    pair_report, "agent_1", "?"
                )
                b = getattr(pair_report, "agent_b", None) or getattr(
                    pair_report, "agent_2", "?"
                )
        ...
        collusive_ids_infer = [
            aid for aid in orchestrator._agents if aid.startswith("collusive_voter")
        ]
    ...
    for atype in ["Diligent", "Spam Bot", "Human Pretender", "Collusive Voter"]:
        ...
        print(
            "  {:<20s}  pass={:4d}  fail={:4d}  total={:4d}  rate={:.2%}".format(
    ...
    pretender_ch_total = pretender_ch_pass + challenge_fail_by_type.get(
        "Human Pretender", 0
    )
    ...
    print()
    print(
        "  CAPTCHA effectiveness against pretenders (diligent - pretender): {:+.2%}".format(
            diligent_rate - pretender_rate
        )
    )
    print(
        "  CAPTCHA effectiveness against spam bots  (diligent - spam):      {:+.2%}".format(
    ...
    if spam_rate > diligent_rate - 0.05:
        print()
        print(
            "  NOTE: Spam bots pass challenges at nearly the same rate as diligent agents."
        )
        print(
            "  This is expected -- spam bots are 'AI' agents with high parse/solve accuracy."
        )
        print(
            "  The CAPTCHA is 'anti-human', so it primarily filters human pretenders."
        )
    ...
    if total_rl > 0:
        ...
        target_msg = (
            "primarily targeting spam bots"
            if spam_rl / total_rl > 0.8
            else "spread across agent types"
        )
    ...
    avg_rep = {}
    for label, ids in [
        ("Diligent", diligent_ids),
        ("Spam Bot", spam_ids),
        ("Human Pretender", pretender_ids),
        ("Collusive Voter", collusive_ids),
    ]:
        avg_rep[label] = (
            statistics.mean(rep_by_agent.get(a, 0) for a in ids) if ids else 0
        )
    ...
    for em in epoch_metrics_list:
        print(
            "  {:5d}  {:12d}  {:8d}  {:8.4f}  {:+8.4f}  {:9.4f}  {:9.2f}".format(
                em.epoch,
                em.total_interactions,
                em.accepted_interactions,
                em.toxicity_rate,
                em.quality_gap,
                em.avg_payoff,
                em.total_welfare,
    ...
    if toxicity >= 0.3:
        ...
        findings.append(
            "System toxicity is low ({:.2%}), indicating strong filtering.".format(
                toxicity
            )
        )
    ...
    if gg_pretender > 0.05:
        print(
            "  [PASS] CAPTCHA + rate limits create a clear payoff gap against human pretenders."
        )
        effective_count += 1
    else:
    ...
    if rate_limit_by_type.get("Spam Bot", 0) > 0 and gg_spam > 0:
        print(
            "  [PASS] Rate limits impose friction on spam bots, resulting in lower payoffs."
        )
        effective_count += 1
    elif gg_spam > 0:
        print(
            "  [PARTIAL] Spam bots earn less than diligent agents, but mainly due to quality differences."
        )
        effective_count += 0.5
    else:
    ...
    if avg_rep["Collusive Voter"] <= avg_rep["Diligent"]:
        print(
            "  [PASS] Collusion detection keeps collusive voter reputation below honest levels."
        )
        effective_count += 1
    else:
        print(
            "  [FAIL] Collusive voters exploit vote exchange to inflate reputation above honest agents."
        )

    print()
    score = effective_count / total_checks * 100
    print(
        "  GOVERNANCE EFFECTIVENESS SCORE: {:.0f}% ({}/{} checks passed)".format(
    ...
    if score >= 80:
        print(
            "  VERDICT: The Moltbook CAPTCHA + rate limiting + collusion detection governance"
        )
        print(
            "  stack effectively distinguishes between honest and adversarial agents."
        )
    elif score >= 50:
