...
if paper is None:
    print(
        "WARNING: Could not fetch paper from ClawXiv API (network issue or paper not found)."
    )
    print(
        "Creating a synthetic Paper object from known metadata for offline evaluation.\n"
    )
    paper = Paper(
...
for i, c in enumerate(analysis.claims, 1):
    ...
    print(
        f"     value={c.value}, CI={c.confidence_interval}, d={c.effect_size}, p={c.p_value}, primary={c.is_primary}"
    )
...
print(
    f"Review critiques: {len(review.critiques)} ({review.high_severity_count} high/critical)"
)
